...
class WanVideoAgentTest(unittest.TestCase):
    ...
    def test_default_animation_prompts_are_specific_and_loopable(self) -> None:
        ...
        self.assertIn("章鱼或触手生物", wan_video_agent.wan_prompt_for_animation("walk_right"))
        self.assertIn("软体动物", wan_video_agent.wan_prompt_for_animation("walk_right"))
        self.assertIn("腕足交替支撑爬行", wan_video_agent.wan_prompt_for_animation("walk_left"))
        self.assertIn("鱼类", wan_video_agent.wan_prompt_for_animation("walk_right"))
    ...
    def test_generate_wan_video_url_uses_wan_plus_model(self) -> None:
        calls = {}
        ...
        class FakeVideoSynthesis:
            @staticmethod
            def async_call(**kwargs: object) -> FakeResponse:
                calls["async_call"] = kwargs
                return FakeResponse(types.SimpleNamespace(task_id="task-1"))

            @staticmethod
            def wait(response: FakeResponse) -> FakeResponse:
                calls["wait_response"] = response
